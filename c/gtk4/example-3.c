...
/* Surface to store current scribbles(涂鸦) */
static cairo_surface_t *surface = NULL;

static void clear_surface(void)
{
  /**
   * GTK takes care of most of the details by providing a ready-to-use cairo context to the draw function
   */
  cairo_t *cr;
...
/* Create a new surface of the appropriate(适当的) size to store our scribbles */
 ...
 * Note that the draw callback receives a ready-to-be-used cairo_t
 * that is already clipped(修剪) to only draw the exposed areas of the widget
 */
...
/* Draw a rectangle(矩形) on the surface at the given position */
static void draw_brush(GtkWidget *widget, double x, double y)
{
  ...
  cairo_destroy(cr);
  /**
   * Now invalidate(使无效) the drawing area.
   * It is also possible to explicitly cause a widget to be redrawn, by calling gtk_widget_queue_draw()
   */
...
static void activate(GtkApplication *app, gpointer user_data)
{
  ...
  GtkWidget *frame;
  /**
   * Sometimes, it is necessary to do some custom drawing.
   * In that case, a GtkDrawingArea might be the right widget to use
   */
