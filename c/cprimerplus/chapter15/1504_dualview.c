 ...
 * 位字段和按位运算符是两种可替换的方法
 * 	如1503中使用unsigned int 类型大小相同的结构储存图形框的信息，也可以使用unsigned int变量储存相同的信息
 * 	如果不想用结构成员表示法来访问不同的部分，也可以使用按位运算符来操作，只是比较麻烦
 *
 */
#include <stdio.h>
#include <stdbool.h>
...
int main(void)
{
	//创建View联合，并初始化initialize struct box view
